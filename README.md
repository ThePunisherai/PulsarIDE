...
| **Toolkit** | One page for what is actually wired into your agents, read back from your machine rather than claimed: whether the tracker server really starts (it is *launched*, not just found) and with which command, which config file each of the seven agents keeps and whether our servers are named in it, which agents keep the board current by themselves, and whether your agents fit Claude Code's 15k agent-description cap (counted its way, with stale roster copies removed on every launch) — plus Meshy's key, ECC's switch, and the Unreal Engine MCP |
