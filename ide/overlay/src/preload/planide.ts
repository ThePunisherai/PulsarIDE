...
export const planIdeApi = {
  ...
  agentBudget: <T>(path?: string) => call<T>('planide:agent-budget', path),
  agentBudgetPrune: <T>(path?: string) => call<T>('planide:agent-budget-prune', path),
  ...
  graphPicture: <T>(path: string) => call<T>('planide:graph-picture', path),
