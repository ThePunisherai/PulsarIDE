...
function mainSessionBlock(home: string): string {
  return [
    ...
    '',
    'Types: architecture, workflow, sequence, dataflow, lifecycle. Write the JSON to',
    '`<project>/.planide/diagrams/<name>.<type>.json` and render the HTML beside it — the',
    'IDE\'s Archify tab lists exactly that directory, so a diagram put anywhere else is',
    'invisible. `validate` the JSON before rendering; its errors name the offending path.',
    'Read `<archify>/SKILL.md` and the JSON files in `<archify>/examples/` for the schema.',
    'Diagram what the code really does — inspect it first; an invented topology drawn',
    'beautifully is worse than no diagram. Put the diagram on the board like any other work',
    '(`add_item` when you plan it, `works` when it renders), and say where you put it.',
    '',
