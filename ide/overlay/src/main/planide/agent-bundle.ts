...
function mainSessionBlock(home: string): string {
  return [
    ...
    '',
    '  3D / shader / animated visual   ThreeUI components (44, see below) -- never write',
    '                                  WebGL by hand before looking at INDEX.md there',
    ...
    '',
    '  node <archify>/bin/archify.mjs render <type> <input.json> <output.html> --repo-root <project>',
    '',
    'Types: architecture, workflow, sequence, dataflow, lifecycle. The schema is strict',
    '(`schema_version`, `diagram_type`, `meta`, `components`, ...), so do not invent it:',
    'copy the matching `<archify>/examples/*.<type>.json` as your exact starting shape and',
    'replace its contents with this project\'s real topology. Then, in order:',
    '',
    '  node <archify>/bin/archify.mjs validate <type> <input.json> --json   # must pass first',
    '  node <archify>/bin/archify.mjs render   <type> <input.json> <output.html> --repo-root <project>',
    '',
    'Write the JSON to `<project>/.planide/diagrams/<name>.<type>.json` and render the HTML',
    'beside it (same base name, `.html`) — the IDE\'s Archify tab lists exactly that',
    'directory, so a diagram put anywhere else is invisible, and one that fails `validate`',
    'never renders. Diagram what the code really does — inspect it first; an invented',
    'topology drawn beautifully is worse than no diagram. Put the diagram on the board like',
    'any other work (`add_item` when you plan it, `set_item` `works` when it renders).',
    '',
