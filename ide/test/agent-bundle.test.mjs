...
// Only the CORE generalist leads deploy natively (manifest.core_team_leads) -- deploying all
// 100 blew Claude Code's ~15k agent-description budget (~36.7k observed). Read the expected
...
ok('force redeploys without accumulating', r3.deployed === true && readdirSync(join(HOME, '.claude/agents')).filter(f => f.startsWith('pulsar-')).length === CORE && existsSync(join(HOME, '.claude/agents/my-own.md')))
