...
> **PulsarIDE bundle note:** the optional ~17 MB royalty-free `assets/` media pack is
> NOT bundled here (see `ATTRIBUTION.md`). Cue presets are read "if present" and
> otherwise detected at composition time; Hyperframes selects/detects the audio. This
> skill needs Hyperframes (`hyperframes.heygen.com`) installed to actually render.
# /brag
